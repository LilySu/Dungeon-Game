...
class World:
    ...
    def generate_rooms(self, size_x, size_y, num_rooms):
        ...
        self.width = size_x #define size of grid
        self.height = size_y

        for i in range(len(self.grid)):
            self.grid[i] = [None] * size_x #create grid
        ...
        if self.grid[0] is None and self.grid[0][0] is None: # plot starting room in middle of grid
            """
            Create genesis grid point/room
            """
            # Generate bewteen 1-4 directions
            direction_amount = math.floor(random.random() * 4 + 1) # add each direction into queue
            ...
            # Create first room
            randomSel = random.randint(0, len(listy)) # start from genesis point, randomly calculate 1-4 directions
